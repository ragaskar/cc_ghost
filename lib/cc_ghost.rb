...
module CcGhost
  ...
  class User < Struct.new(:guid, :email); end
  class Space < Struct.new(:name, :guid); end

  class TypeFactory
    def organization(obj)
      CcGhost::Organization.new(obj.name, obj.guid)
    end

    def user(obj)
      CcGhost::User.new(obj.guid, 'some-random-email@blah.com')
    end

    def space(obj)
      CcGhost::Space.new(obj.name, obj.guid)
    end
  end
  ...
  class Ghostroller
    ...
    def initialize
      @fork = Fork.new :to_fork, :from_fork, :return do |fork|
        begin
